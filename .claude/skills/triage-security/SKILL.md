...
**What this repository has that the crate repositories do not:** three
container images and a chart, all signed. A vulnerable dependency reaches
users as a *layer* here, not as a source tree, so a fix is not shipped until
the images are rebuilt and re-signed — which means a release, not a merge.
The Dockerfile's bases are pinned by digest and Dependabot watches the
`docker` ecosystem for them; a base bump is the same triage as a crate bump
